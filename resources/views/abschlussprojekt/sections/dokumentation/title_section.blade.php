...
<div class="form-group row">
    ...
    <input type="text" form="{{ $form }}" name="shortTitle" id="{{ $v_name }}shortTitle" @if($disable) disabled @else placeholder="kurzer Titel" @endif
           class="form-control col-lg-10 @error('shortTitle') is-invalid @enderror" value="{{ old("shortTitle") ?: $documentation->getShortTitle($version) }}"/>
...
<div class="form-group row">
    ...
    <input type="text" form="{{ $form }}" name="longTitle" id="{{ $v_name }}longTitle" @if($disable) disabled @else placeholder="langer Titel" @endif
           class="form-control col-lg-10 @error('longTitle') is-invalid @enderror" value="{{ old("longTitle") ?: $documentation->getLongTitle($version) }}"/>
